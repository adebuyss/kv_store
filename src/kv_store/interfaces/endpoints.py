...
def get(key: str):
    ...
    try:
        value = store.get(key)
        return Response(response=json.dumps({'value': value}), status=200, mimetype='application/json')
    except KeyError:
        abort(404)
...
def insert():
    ...
    if store.exists(key):
        return Response(response=json.dumps({'error': 'duplicate key'}), status=409, mimetype='application/json')
