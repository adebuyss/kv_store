...
def make_request(endpoint, data, request_func):
    ...
    logger.info(
        f"Calling api - Url: {url} Headers: {headers} Data: {data} Type: {request_func.__name__}"
    )
    response = request_func(url=url, data=data, headers=headers)
    logger.info(
        f"Api response - Status: {response.status_code}, Content: {response.content}"
    )
    return response
...
def set_key_post(keyname, value):
    return make_request(
        f"v1/", {"key": keyname, "value": value}, requests.post
    ).status_code
